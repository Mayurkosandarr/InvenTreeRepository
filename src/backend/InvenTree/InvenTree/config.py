...
def get_media_dir(create=True):
    """Return the absolute path for the 'media' directory."""
    media_root = os.getenv('INVENTREE_MEDIA_ROOT')

    if not media_root:
        # Default to a known media directory
...
def get_static_dir(create=True):
    """Return the absolute path for the 'static' directory."""
    sd = os.getenv('INVENTREE_STATIC_ROOT', 'static_root')
...
def get_backup_dir(create=True):
    ...
    backup_dir = os.getenv('INVENTREE_BACKUP_DIR')

    if not backup_dir:
        # Default to a known backup directory
