...
import Quotation from '../../components/nav/quotation';

export default function AcceptedQuotationTable({
  partId,
  customerId
}: Readonly<{
  partId?: number;
  ...
  const fetchQuotations = async () => {
    try {
      ...
      const result = await response.json();
      if (response.ok) {
        const acceptedQuotations = result.filter((quotation: any) => quotation.status === 'accepted');
