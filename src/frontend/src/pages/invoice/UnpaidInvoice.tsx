...
export function UnPaidInvoiceTable({
  ...
  // Create a refresh function that updates both invoices and leads
  const refreshData = useCallback(async () => {
    try {
      table.setIsLoading(true);
      await Promise.all([fetchInvoices(), fetchLeads()]);
    } finally {
      table.setIsLoading(false);
    }
  }, [table]);

  // Override the default refreshTable function
  useEffect(() => {
    table.refreshTable = refreshData;
  }, [refreshData, table]);
